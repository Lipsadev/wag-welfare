...
const RescueList = forwardRef((props: RescueListProps, ref) => {
  ...
  const fetchRescues = async () => {
    try {
      ...
      console.log("Fetched rescues:", data); // Debug log
      // Handle both { rescues: [...] } and direct array
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg w-full rounded-lg p-6 relative">
          ...
          {selectedRescue && (
            <div>
              <img
                src={`http://localhost:5000/uploads/${selectedRescue.image}`}
              ...
              <p className="text-gray-700">
                <span className="font-semibold">Adoptions:</span> {selectedRescue.adoptionCount || 0}
              </p>
            </div>
          )}
        </DialogContent>
